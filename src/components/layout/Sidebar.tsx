...
import { LayoutDashboard, ClipboardList, Hotel, AlertCircle, Settings, RotateCcw } from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside aria-label="Sidebar navigation" className="w-64 bg-base-100 border-r border-base-200 flex flex-col justify-between h-screen sticky top-0 shrink-0 hidden md:flex">
      <div>
        {/* Brand Header */}
        <div className="p-5 border-b border-base-200 flex items-center gap-3">
          <div className="p-2.5 bg-primary text-primary-content rounded-xl shadow-md" aria-hidden="true">
        ...
        <nav aria-label="Main navigation" className="p-4 space-y-1">
          ...
          >
            ...
            {pendingCount > 0 && (
              <span className="badge badge-sm font-bold bg-amber-500 text-white border-none" aria-label={`${pendingCount} pending orders`}>
                {pendingCount}
              </span>
            )}
          </NavLink>
        ...
        {attentionCount > 0 && (
          <div role="region" aria-live="assertive" className="mx-4 my-2 p-3 bg-rose-50 border border-rose-200 rounded-xl flex items-center gap-2.5">
            <AlertCircle className="w-5 h-5 text-rose-600 shrink-0 animate-bounce" aria-hidden="true" />
            <div>
              <p className="text-xs font-bold text-rose-900">
                {attentionCount} Need{attentionCount === 1 ? 's' : ''} Attention
              </p>
              <p className="text-[11px] text-rose-700">Check priority in Orders</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="p-4 border-t border-base-200 space-y-3">
        {/* User Card */}
        <div className="flex items-center gap-3 px-2 py-1" aria-label="Current logged in staff user">
          <div className="avatar placeholder" aria-hidden="true">
            <div className="bg-neutral text-neutral-content rounded-full w-8 font-bold text-xs">
              JD
            </div>
          </div>
        ...
        <details className="group border border-base-200 rounded-xl bg-base-200/40 text-xs overflow-hidden">
          <summary className="flex items-center justify-between p-2.5 cursor-pointer font-medium text-base-content/60 hover:text-base-content hover:bg-base-200/80 transition-colors select-none">
            <span className="flex items-center gap-1.5 font-semibold text-[11px]">
              <Settings className="w-3.5 h-3.5 text-base-content/50" aria-hidden="true" />
              Dev Tools
            </span>
            <span className="text-[10px] text-base-content/40 group-open:rotate-180 transition-transform">▼</span>
          </summary>
